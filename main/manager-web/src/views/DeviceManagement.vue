              ...
              <el-table-column label="备注" align="center">
                <template slot-scope="scope">
                  <el-input
                    ...
                    size="mini"
                    @blur="stopEditRemark(scope.$index, scope.row)"
                  ></el-input>
                  <span v-else>
                    <i
                      v-if="!scope.row.remark"
                      class="el-icon-edit"
                      @click="startEditRemark(scope.$index, scope.row)"
                    ></i>
                    <span
                      v-else
                      @click="startEditRemark(scope.$index, scope.row)"
...
export default {
    ...
    filteredDeviceList() {
      ...
      return this.deviceList.filter(
        (device) =>
          ...
          (device.macAddress &&
            device.macAddress.toLowerCase().includes(keyword))
      );
    },
  ...
  methods: {
    ...
    handleSelectAll() {
      ...
      this.selectedDevices = this.paginatedDeviceList.filter(
    ...
    deleteSelected() {
      ...
      this.$confirm(
        `确认要解绑选中的 ${this.selectedDevices.length} 台设备吗？`,
        "警告",
        {
        ...
        const deviceIds = this.selectedDevices.map(
    ...
    stopEditRemark(index, value) {
      this.deviceList[index].isEdit = false;
      this.deviceList[index].remark = value.remark;
      // macAddress
      Api.device.updateRemark(value.macAddress, value.remark, ({ data }) => {
        if (data.code === 0) {
          this.deviceList[index].remark = value.remark;
      ...
      console.log("this.deviceList[index]", value);
    },
    ...
    handleOtaSwitchChange(row) {
      Api.device.enableOtaUpgrade(
        row.device_id,
        row.otaSwitch ? 1 : 0,
        ({ data }) => {
          if (data.code === 0) {
            this.$message.success(
              row.otaSwitch ? "已设置成自动升级" : "已关闭自动升级"
            );
          } else {
