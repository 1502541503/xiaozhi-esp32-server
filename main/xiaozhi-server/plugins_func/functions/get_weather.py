...
def get_weather(conn, location: str = "深圳", lang: str = "zh_CN"):
    from core.utils.cache.manager import cache_manager, CacheType

    logger.info(f"进入天气插件: ")
    logger.info(f"AI解析用户询问位置信息: {location}")
    ...
    logger.info(f"解析用户咨询的城市信息:{city_info}")
    if not city_info:
    ...
    logger.info(f"解析用户位置最终城市结果: {final_location}")
    logger.info(f"解析用户位置最终经纬度结果: {final_lat}, {final_lon}")
    logger.info(f"天气插件开始请求缓存数据：{cached_weather_report}")
